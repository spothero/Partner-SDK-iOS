...
protocol ShowTimeSelectionViewDelegate: AnyObject {
    func timeSelectionViewShouldShow(_ show: Bool)
    func didPressEndDoneButton()
}

protocol StartEndDateDelegate: AnyObject {
...
class TimeSelectionView: UIView {
    ...
    @IBOutlet private var startsView: UIView!
    @IBOutlet private var endsView: UIView!
    ...
    var startDate = Date().shp_roundDateToNearestHalfHour(roundDown: true) {
        didSet {
    ...
    var endDate = Date().addingTimeInterval(Constants.SixHoursInSeconds).shp_roundDateToNearestHalfHour(roundDown: true) {
        didSet {
    ...
    @objc
    private func startDateChanged(_ datePicker: UIDatePicker) {
        self.setStartEndDateTimeLabelWithDate(datePicker.date)
    }
    
    @objc
    private func endDateChanged(_ datePicker: UIDatePicker) {
        self.setStartEndDateTimeLabelWithDate(datePicker.date)
    }
