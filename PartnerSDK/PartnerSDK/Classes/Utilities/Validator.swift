...
enum Validator {
    /**
     ...
     - Returns: a bool indicating if the email is valid
     - Throws: throws an error if string is empty or invalid
     */
    @discardableResult
    static func validateEmail(_ email: String?) throws -> Bool {
        let fieldName = LocalizedStrings.Email
        let message = LocalizedStrings.EmailErrorMessage
        
        guard
            let trimmedEmail = email?.sph_trimmedOfAllWhitespace(),
    ...
    /**
     ...
     - returns: Whether the license is valid or not
     */
    static func validateLicense(_ license: String) -> Bool {
        let trimmedLicense = license.trimmingCharacters(in: CharacterSet.whitespacesAndNewlines)
        
        let validLicensePlateCharacters = LicensePlateFormatter.ValidLicensePlateCharacters
        let maxPlateCharacters = LicensePlateFormatter.MaxPlateCharacters
